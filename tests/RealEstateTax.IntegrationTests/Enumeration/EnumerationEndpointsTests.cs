...
public class EnumerationEndpointsTests : IAsyncLifetime
{
    ...
    [Fact]
    public async Task GetUnmatchedRecords_WithCitizenRole_Returns403()
    {
        ...
        var response = await citizenClient.GetAsync("/api/enumeration/unmatched-records");
        response.StatusCode.Should().Be(HttpStatusCode.Forbidden);
    }


    [Fact]
    public async Task GetUnmatchedRecords_WithFieldInspectorRole_Returns403()
    {
        ...
        var response = await inspectorClient.GetAsync("/api/enumeration/unmatched-records");
        response.StatusCode.Should().Be(HttpStatusCode.Forbidden);
    }
    ...
    public async Task GetDataQualityIssues_WithCitizenRole_Returns403()
    {
        ...
        var response = await citizenClient.GetAsync("/api/enumeration/data-quality-issues");
        response.StatusCode.Should().Be(HttpStatusCode.Forbidden);
    }
    ...
    public async Task ImportSourceRecords_WithCitizenRole_Returns403()
    {
        ...
        var response = await citizenClient.PostAsJsonAsync("/api/enumeration/import-source-records", new
        {
            source = "NUCA",
            records = Array.Empty<object>()
        });
