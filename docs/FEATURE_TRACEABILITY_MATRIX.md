...
| Billing workflow | `GET/POST /api/bills` | billing app services | `BillsPage.tsx` | `TaxCalculationServiceTests` (related liability logic) | `BillsEndpointsTests` | L2 | Settlement edge cases: partial→full, overpayment handling | QA Automation | In Progress |
| Payment settlement | `GET/POST /api/payments` | payment app services | `PaymentsPage.tsx` | `TaxCalculationServiceTests` (balance impact assertions) | `PaymentsEndpointsTests` | L2 | Duplicate-payment and reconciliation branch coverage | QA Automation | In Progress |
| Appeals workflow | `GET/POST /api/appeals` | appeal app services | `AppealsPage.tsx` | `AppealValidators` coverage pending dedicated tests | `AppealsController` integration coverage pending | L1 | Deadline/legal rule validation + payable impact scenarios | Product + Legal | In Progress |
| Exemption processing | `GET/POST /api/exemptions` | `ExemptionDomainService` | `ExemptionsPage.tsx` | `ExemptionDomainServiceTests` | `ExemptionsController` integration coverage pending | L1 | Legal article mapping + combined payable effects | Product + Legal | In Progress |
| Valuation & assessments | `GET/POST /api/valuations`, `GET/POST /api/taxassessments` | `ValuationDomainService`, `TaxCalculationService` | `ValuationsPage.tsx` | `ValuationDomainServiceTests`, `TaxCalculationServiceTests` | `TaxAssessmentsController` integration coverage pending expansion | L2 | Approval-flow + recalculation branch coverage | Backend Lead | In Progress |
