...
public class AppealsEndpointsTests : IAsyncLifetime
{
    ...
    public async Task GetById_WithTaxOfficerRole_Returns403()
    {
        ...
        var response = await officerClient.GetAsync($"/api/appeals/{Guid.NewGuid()}");
        response.StatusCode.Should().Be(HttpStatusCode.Forbidden);
    }
    ...
    public async Task GetById_WithTaxAssessorRole_Returns403()
    {
        ...
        var response = await assessorClient.GetAsync($"/api/appeals/{Guid.NewGuid()}");
        response.StatusCode.Should().Be(HttpStatusCode.Forbidden);
    }
    ...
    [Fact]
    public async Task Decision_WithTaxAssessorRole_Returns403()
    {
        var creds = await _factory.CreateUserWithRoleAsync("TaxAssessor");
        var assessorClient = await AuthenticatedHttpClient.CreateAsync(_factory, creds.Username, creds.Password);

        var response = await assessorClient.PostAsJsonAsync($"/api/appeals/{Guid.NewGuid()}/decision", new
        {
            decision = "Rejected",
            reason = "role guard check"
        });
