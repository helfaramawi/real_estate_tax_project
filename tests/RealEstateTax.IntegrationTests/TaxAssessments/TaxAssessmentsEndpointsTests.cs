...
public class TaxAssessmentsEndpointsTests : IAsyncLifetime
{
    ...
    public async Task GetByProperty_WithCitizenRole_Returns403()
    {
        ...
        var response = await citizenClient.GetAsync($"/api/tax-assessments/property/{Guid.NewGuid()}");
    ...
    public async Task GetByProperty_WithFieldInspectorRole_Returns403()
    {
        ...
        var response = await inspectorClient.GetAsync($"/api/tax-assessments/property/{Guid.NewGuid()}");
        response.StatusCode.Should().Be(HttpStatusCode.Forbidden);
    }


    [Fact]
    public async Task GetByProperty_WithTaxOfficerRole_Returns403()
    {
        ...
        var response = await officerClient.GetAsync($"/api/tax-assessments/property/{Guid.NewGuid()}");
        response.StatusCode.Should().Be(HttpStatusCode.Forbidden);
    }


    [Fact]
    public async Task GetByProperty_WithTaxAssessorRole_Returns403()
    {
        ...
        var response = await assessorClient.GetAsync($"/api/tax-assessments/property/{Guid.NewGuid()}");
        response.StatusCode.Should().Be(HttpStatusCode.Forbidden);
    }
    ...
    [Fact]
    public async Task Approve_WithTaxAssessorRole_Returns403()
    {
        var creds = await _factory.CreateUserWithRoleAsync("TaxAssessor");
        var assessorClient = await AuthenticatedHttpClient.CreateAsync(_factory, creds.Username, creds.Password);

        var response = await assessorClient.PostAsJsonAsync($"/api/tax-assessments/{Guid.NewGuid()}/approve", new
        {
            notes = "role guard check"
        });
