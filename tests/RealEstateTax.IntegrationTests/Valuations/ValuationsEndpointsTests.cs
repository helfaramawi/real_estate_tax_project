...
namespace RealEstateTax.IntegrationTests.Valuations;

[Collection("Integration")]
public class ValuationsEndpointsTests : IAsyncLifetime
{
    private readonly CustomWebApplicationFactory _factory;

    public ValuationsEndpointsTests(CustomWebApplicationFactory factory)
    {
        _factory = factory;
    }
    ...
    public async Task GetByProperty_WithoutToken_Returns401()
    {
        ...
        var response = await unauthenticated.GetAsync($"/api/valuations/property/{Guid.NewGuid()}");
    ...
    public async Task GetByProperty_WithCitizenRole_Returns403()
    {
        ...
        var response = await citizenClient.GetAsync($"/api/valuations/property/{Guid.NewGuid()}");
    ...
    public async Task GetByProperty_WithFieldInspectorRole_Returns403()
    {
        ...
        var response = await inspectorClient.GetAsync($"/api/valuations/property/{Guid.NewGuid()}");
        response.StatusCode.Should().Be(HttpStatusCode.Forbidden);
    }
    ...
    public async Task Approve_WithCitizenRole_Returns403()
    {
        ...
        var response = await citizenClient.PostAsJsonAsync($"/api/valuations/{Guid.NewGuid()}/approve", new
        {
    ...
    public async Task Approve_WithFieldInspectorRole_Returns403()
    {
        ...
        var response = await inspectorClient.PostAsJsonAsync($"/api/valuations/{Guid.NewGuid()}/approve", new
        {
            reviewerNotes = "role guard check"
        });
    ...
    {
        var unauthenticated = _factory.CreateClient();
        var response = await unauthenticated.PostAsJsonAsync($"/api/valuations/{Guid.NewGuid()}/reject", new
        {
    ...
    public async Task Reject_WithCitizenRole_Returns403()
    {
        ...
        var response = await citizenClient.PostAsJsonAsync($"/api/valuations/{Guid.NewGuid()}/reject", new
        {
    ...
    public async Task Reject_WithFieldInspectorRole_Returns403()
    {
        ...
        var response = await inspectorClient.PostAsJsonAsync($"/api/valuations/{Guid.NewGuid()}/reject", new
        {
            reason = "role guard check"
        });
    ...
    [Fact]
    public async Task Reject_WithTaxOfficerRole_Returns403()
    {
        ...
        var response = await officerClient.PostAsJsonAsync($"/api/valuations/{Guid.NewGuid()}/reject", new
        {
            reason = "role guard check"
        });
