...
public class IntegrationsEndpointsTests : IAsyncLifetime
{
    ...
    [Fact]
    public async Task Receive_WithFieldInspectorRole_Returns403()
    {
        ...
        var response = await inspectorClient.PostAsJsonAsync("/api/integrations/NUCA/receive", new
        {
            sourceSystem = "NUCA",
            records = Array.Empty<object>()
        });
    ...
    [Fact]
    public async Task Receive_WithTaxOfficerRole_Returns403()
    {
        ...
        var response = await officerClient.PostAsJsonAsync("/api/integrations/NUCA/receive", new
        {
            sourceSystem = "NUCA",
            records = Array.Empty<object>()
        });
    ...
    public async Task GetRequests_WithCitizenRole_Returns403()
    {
        ...
        var response = await citizenClient.GetAsync("/api/integrations/requests");
        response.StatusCode.Should().Be(HttpStatusCode.Forbidden);
    }
    ...
    [Fact]
    public async Task Retry_WithFieldInspectorRole_Returns403()
    {
        var creds = await _factory.CreateUserWithRoleAsync("FieldInspector");
        var inspectorClient = await AuthenticatedHttpClient.CreateAsync(_factory, creds.Username, creds.Password);

        var response = await inspectorClient.PostAsync($"/api/integrations/requests/{Guid.NewGuid()}/retry", content: null);
        response.StatusCode.Should().Be(HttpStatusCode.Forbidden);
    }
    ...
    [Fact]
    public async Task Retry_WithTaxOfficerRole_Returns403()
    {
        var creds = await _factory.CreateUserWithRoleAsync("TaxOfficer");
        var officerClient = await AuthenticatedHttpClient.CreateAsync(_factory, creds.Username, creds.Password);

        var response = await officerClient.PostAsync($"/api/integrations/requests/{Guid.NewGuid()}/retry", content: null);
        response.StatusCode.Should().Be(HttpStatusCode.Forbidden);
    }
    ...
    [Fact]
    public async Task GetRequests_WithTaxOfficerRole_Returns403()
    {
        var creds = await _factory.CreateUserWithRoleAsync("TaxOfficer");
        var officerClient = await AuthenticatedHttpClient.CreateAsync(_factory, creds.Username, creds.Password);
