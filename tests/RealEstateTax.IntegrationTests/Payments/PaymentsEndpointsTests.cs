...
public class PaymentsEndpointsTests : IAsyncLifetime
{
    ...
    [Fact]
    public async Task GetByBill_WithTaxOfficerRole_Returns403()
    {
        ...
        var response = await officerClient.GetAsync($"/api/payments/bill/{Guid.NewGuid()}");
        response.StatusCode.Should().Be(HttpStatusCode.Forbidden);
    }


    [Fact]
    public async Task GetByBill_WithTaxAssessorRole_Returns403()
    {
        ...
        var response = await assessorClient.GetAsync($"/api/payments/bill/{Guid.NewGuid()}");
        response.StatusCode.Should().Be(HttpStatusCode.Forbidden);
    }
    ...
    [Fact]
    public async Task Register_WithTaxAssessorRole_Returns403()
    {
        var creds = await _factory.CreateUserWithRoleAsync("TaxAssessor");
        var assessorClient = await AuthenticatedHttpClient.CreateAsync(_factory, creds.Username, creds.Password);

        var response = await assessorClient.PostAsJsonAsync("/api/payments", new { taxBillId = Guid.NewGuid(), amount = 100m });
        response.StatusCode.Should().Be(HttpStatusCode.Forbidden);
    }
